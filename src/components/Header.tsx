...
const NavBar = (): JSX.Element => {
  ...
  return (
    ...
    <Router>
      <nav className= {scrolled ? " fixed w-full top-0 z-50  duration-[0.32s] ease-in-out p-0 bg-[#121212]" : "py-[18px] fixed w-full top-0 z-50  duration-[0.32s] ease-in-out "}>
        <div className='flex flex-nowrap items-center justify-between max-w-[390px] sm:max-w-[570px] md:max-w-[720px] lg:max-w-[940px] xl:max-w-[1140px] 2xl:max-w-[1320px]   mr-auto ml-auto'>
          <span className='w-[9%] mr-4 hidden md:block text-sm sm:text-base md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl  font-bold '>
           DMT 
          </span>
        ...
        <div
          className={` md:flex md:items-center md:w-auto   ${isOpen ? "block " : "hidden"}`}
        >
          <div className=" text-sm flex   lg:flex-grow justify-center items-center ">
             <HashLink smooth to="/#Banner" >
             <a href='https'  className={activeLink === 'home' ? 'text-[#fff] font-normal tracking-wider px-[25px] text-lg' : 'text-gray-400 font-normal tracking-wider px-[25px] text-lg'} onClick={() => onUpdateActiveLink('home')}>Home</a>
            </HashLink>
             <HashLink smooth to="/#Skilles">
          ...
          <span className='flex items-center py-2 '>
            ...
            <button className="font-bold text-[#fff] hover:text-[#121212] border border-solid border-white py-[15px] px-[16px] text-lg ml-[18px] relative bg-transparent  duration-300 ease-in-out
      ...
      <div className="block absolute right-0 top-6  md:hidden ">
          <button
